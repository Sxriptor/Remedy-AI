...
export function AuthProvider({ children }: { children: React.ReactNode }) {
    ...
    try {
      ...
      const fallbackUser = {
        ...
        github_id: authUser.user_metadata?.provider_id
          ? parseInt(authUser.user_metadata.provider_id)
          : null,
        github_username:
  ...
  const convertElectronUserToSupabaseUser = useCallback(
    (electronUser: any): User => {
      return {
        id: electronUser.id,
  ...
  useEffect(() => {
    const getInitialSession = async () => {
      try {
        ...
        if (isElectronEnv) {
          ...
          console.log(
            "🖥️ Electron detected, checking auth via Electron API..."
          );
          ...
          if (isAuthenticated) {
            ...
            if (electronUser) {
              const supabaseUser =
                convertElectronUserToSupabaseUser(electronUser);
    ...
    supabase.auth,
    ensureCustomUserExists,
    convertElectronUserToSupabaseUser,
  ]);
  ...
  useEffect(() => {
    if (isElectron && typeof window !== "undefined") {
      ...
      electronAPI.onAuthSuccess(async (data: any) => {
        ...
        try {
          ...
          if (isAuthenticated) {
            const electronUser = await electronAPI.getUser();
            console.log(
              "👤 Got Electron user after auth success:",
              electronUser
            );
            if (electronUser) {
              const supabaseUser =
      ...
      electronAPI.onAuthSignedOut(async () => {
        ...
        try {
          const { resetElectronClient } = await import(
            "@/lib/supabase/electron-client"
          );
          resetElectronClient();
        } catch (error) {
      ...
      const {
        ...
        async (event: AuthChangeEvent, session: Session | null) => {
          try {
            ...
            if (authUser) {
              console.log(
                "👤 Auth user in state change, ensuring custom user exists..."
              );
              await ensureCustomUserExists(authUser);
